...
- **Commit messages and PR descriptions are project documentation, not a work journal.** They become permanent record that a future reader consults to understand why the code is the way it is. Write about the *change*: what it does, what it rules out, what evidence supports it, what was considered and rejected. Do not write about the process of authoring it — no first person, no "review caught this", no tallying how often something has gone wrong, no narration of the session. Rationale belongs in a commit message; autobiography does not. "Three places cited ten designs while eight were committed" is documentation; "I claimed ten designs and was caught" is a diary entry that will outlive its context.
...
- **Never `rm`.** Not with `-f`, not on a file you just created, not to tidy up after yourself. Deletion prompts the user for permission, so every cleanup interrupts them — and a wrong path is unrecoverable. If something needs removing, leave it and say so; the user can delete it in a keystroke. `git clean`, `find -delete`, and `> file` truncation are the same act under different names.
- **Never write scratch files into the repository.** No throwaway `examples/`, no probe scripts, no test fixtures you intend to delete. A file written into the tree has to be removed again, which is the rule above, and one that slips into a commit is worse. Scratch work goes in `/tmp/`, and stays there — do not clean it up either.
- **Scratch tooling that needs the crates** goes in a standalone cargo project under `/tmp` with `path` dependencies pointing back at `crates/`. That dogfoods the real code — an oracle result decoded by deCPLD's own parser is worth more than one decoded by an ad-hoc script, and an ad-hoc JEDEC regex has already produced a wrong answer once.
- **To test whether a path is ignored, ask git rather than creating the file.** `git check-ignore -v <path>` works on paths that do not exist.
