...
- [ ] Decode: fuse vector → `PhysicalDesign` — `decode_row` recovers a product term from fuses in [#37](https://github.com/enthal/decpld/pull/37); the whole-design shape needs `MacrocellSpec`
- [ ] Invariants: matrix cells map 1:1 to fuses, no undocumented field overlap, every fuse classified, every legal configuration round-trips — cells-to-fuses and fuse classification are construction-time in [#28](https://github.com/enthal/decpld/pull/28) and [#37](https://github.com/enthal/decpld/pull/37); macrocell configurations follow
