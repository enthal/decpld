...
- [x] Differential experiment suite: literal mapping, polarity, registered, feedback, macrocell, fuse-count modes, signature — [#30](https://github.com/enthal/decpld/pull/30); pin roles, shared clock/input, I/O-as-input, and expected-failure supply-rail designs in [#36](https://github.com/enthal/decpld/pull/36); output enable and bidirectional readback in [#41](https://github.com/enthal/decpld/pull/41). SPEC.md §7.4's baseline group (blank, constant zero, constant one, each output pin), its remaining registered items (`Y.D=!Y`, registered feedback, hold mux), and capacity are still to do
