...
- [ ] `decpld-device`: `DeviceTarget` trait, `FuseMap` with write tracking, `FuseRegion`, `ConfigField`, `AndMatrixSpec`, `MacrocellSpec`, `PackageSpec` — `FuseMap`, `FuseRegions`, `FuseId`, `FuseMutability` landed in [#28](https://github.com/enthal/decpld/pull/28); `PackageSpec`, `PackagePin`, `PinNumber`, `PadId`, `ClockResourceId`, `InputResourceId`, `PowerRail` in [#36](https://github.com/enthal/decpld/pull/36); `AndMatrixSpec`, `MatrixCellSpec`, `LiteralSource`, `ProductTermSpec`, `MacrocellId` and atomic `FuseMap::set_all` in [#37](https://github.com/enthal/decpld/pull/37); `ConfigField` and `MacrocellSpec` in [#38](https://github.com/enthal/decpld/pull/38); `PhysicalDesign`, `MacrocellConfig`, `PlacedCube`, `disable_row`, `row_is_never_true` in [#39](https://github.com/enthal/decpld/pull/39); `DeviceTarget` follows, with a target-associated `DesignError` rather than §4.4's row error types
- [x] `decpld-atf22v10`: DIP-24 package map, global clock, AND matrix, ten macrocells — every field citing its evidence. Fuse regions, array geometry, column and row mapping, and the three JEDEC footprints landed in [#31](https://github.com/enthal/decpld/pull/31); the array's fuse addressing, measured rather than assumed, in [#35](https://github.com/enthal/decpld/pull/35); the DIP-24 package map and the global clock in [#36](https://github.com/enthal/decpld/pull/36); the AND matrix in [#37](https://github.com/enthal/decpld/pull/37); the ten macrocells with their measured polarity and mode fields in [#38](https://github.com/enthal/decpld/pull/38)
- [x] Encode: cube encoding with contradiction rejection, then row decode-and-verify — [#37](https://github.com/enthal/decpld/pull/37). `encode_cube` refuses contradictions and unroutable literals before writing anything, applies the row atomically, then decodes and compares canonical cubes
- [x] Decode: fuse vector → `PhysicalDesign` — [#39](https://github.com/enthal/decpld/pull/39). Whole-design encode and decode round-trip, and encoding reproduces the oracle's entire array for `in2` and `global-ar-sp`
- [x] Invariants: matrix cells map 1:1 to fuses, no undocumented field overlap, every fuse classified, every legal configuration round-trips — cells-to-fuses and fuse classification are construction-time in [#28](https://github.com/enthal/decpld/pull/28) and [#37](https://github.com/enthal/decpld/pull/37); every legal macrocell configuration round-trips in [#38](https://github.com/enthal/decpld/pull/38); whole designs round-trip, and a design must describe every macrocell because an unconfigured one leaves its rows erased — that is, constantly true — in [#39](https://github.com/enthal/decpld/pull/39)
- [x] `decpld jed inspect --device ATF22V10C` with a `--json` form — [#40](https://github.com/enthal/decpld/pull/40). `FuseMap::from_states` loads a vector this compiler did not write, `decpld-report` renders a `PhysicalDesign` in pins, and the CLI joins the three layers without any of them learning about the others
...
- [x] Differential experiment suite: literal mapping, polarity, registered, feedback, macrocell, fuse-count modes, signature — [#30](https://github.com/enthal/decpld/pull/30); pin roles, shared clock/input, I/O-as-input, and expected-failure supply-rail designs in [#36](https://github.com/enthal/decpld/pull/36); output enable and bidirectional readback in [#41](https://github.com/enthal/decpld/pull/41). capacity and row ownership for three of the ten macrocells — both eight-term ends and the widest — in [#43](https://github.com/enthal/decpld/pull/43). SPEC.md §7.4's baseline group (blank, constant zero, constant one, each output pin) and its remaining registered items (`Y.D=!Y`, registered feedback, hold mux) are still to do
- [x] `decpld oracle diff` with delta classification — [#42](https://github.com/enthal/decpld/pull/42). `FuseMeaning` and `classify_fuse` live in `decpld-device` and introduce no device knowledge: everything they consult is already in the matrix, the macrocells, and the regions, so every target gets classification for free
- [x] Evidence level recorded per mapping; `targets/evidence/` populated — ATF22V10 fuse map measured and recorded in [#30](https://github.com/enthal/decpld/pull/30), with the complete fuse-address map added in [#35](https://github.com/enthal/decpld/pull/35); levels in [#44](https://github.com/enthal/decpld/pull/44), as a `Mapping` table whose citations are checked against the experiment suite on disk and against the evidence document's own `Experiments:` lines, rather than only written in comments. The unit is the mapping, not the struct field: no `ConfigFieldId` or `FuseId` carries a level of its own, and attaching one per field is future work
