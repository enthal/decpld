...
- **Not in the standard** — since the two tables partition the alphabet, this means the field did not begin with a letter. Such a field is reported (error in strict, warning otherwise) **and retained**, so a rewrite still emits it. Discarding it was silent data loss in the default mode.
