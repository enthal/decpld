...
- `MacrocellMode::InputOnly` is **supported but not encodable**: the architecture region is exactly two fuses per macrocell and both are spoken for by polarity and mode, so an undriven cell is indistinguishable in those bits from a combinational active-low output. What it *is* distinguishable by is the output-enable term, and §7.4's experiments measure the encoding: asked for a permanently disabled output, the oracle writes an enable row with every link intact and changes nothing else in 5892 fuses — architecture bits and data row included — while an explicitly always-enabled output is bit-identical to one that says nothing about the enable. That the term is what physically gates the pad is the reading of that encoding rather than a separate measurement, and it additionally assumes feedback is taken at the pad rather than at an internal node; neither is `HardwareVerified`. So `MacrocellSpec::supports` reports the capability, the mode field refuses to encode it, and a fitter reaches for the output-enable term rather than the field;
